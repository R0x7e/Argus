...
| **14 款安全工具** | 覆盖被动侦察、主动探测、PoC 执行全阶段，四级风险管控 |
...
│   │   │   │   ├── search_tree.py    #   MCTS 搜索树（Wilson UCB、回传、剪枝）
│   │   │   │   ├── react_executor.py #   ReAct 循环执行器（并发池）
│   │   │   │   ├── expansion_engine.py # 发现驱动的动态扩展引擎
│   │   │   │   ├── shared_knowledge.py # 跨分支共享知识库
│   │   │   │   ├── multi_level_prober.py # 多级探测器（Level 0 快速探测）
│   │   │   │   ├── reward.py         #   奖励函数（信号设计）
│   │   │   │   ├── actions.py        #   动作空间定义与执行
│   │   │   │   ├── param_fuzzer.py   #   参数模糊测试
│   │   │   │   ├── payload_library.py #   Payload 库
...
│   ├── api_wrapper.py               #   FastAPI HTTP API 包装
...
```
MCTS Loop:
  1. Select   — 自适应多因素选择（Wilson UCB + 多样性 + 新近度 + 先验）
  2. Execute  — ReAct 循环执行具体验证（Thought → Action → Observation）
  3. Expand   — 发现驱动的动态扩展（新端点、WAF 规则、参数推断）
...
| `sql_injection` | SQL 注入检测（时间盲注、报错注入等） | L1 |
...
| `JWT_SECRET` | 生产环境必需 | `your-super-secret-key-change-in-production` | JWT 签名密钥 |
| `DATABASE_URL` | 否 | `postgresql+asyncpg://argus:argus_dev_password@localhost:5432/argus` | PostgreSQL 连接串 |
| `REDIS_URL` | 否 | `redis://localhost:6379/0` | Redis 连接地址 |
| `NATS_URL` | 否 | `nats://localhost:4222` | NATS 消息总线地址 |
...
| `SIDECAR_SECRET` | 否 | - | Sidecar 服务共享密钥（用于内部服务认证） |
...
class MyNewTool(BaseTool):
    name = "my_tool"
    description = "工具描述"
    risk_level = RiskLevel.L1  # L0: 只读, L1: 主动探测, L2: 漏洞利用
...
2. 在 `backend/app/tools/__init__.py` 的 `_register_all_tools()` 中注册工具
