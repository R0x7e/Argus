...
- sqli_detect(url, param, method): 【SQLi 专用】使用专用 SQL 注入检测工具，自动执行错误型检测（14 种 SQL 错误模式）和时间盲注检测（SLEEP 对比）。当节点漏洞类型为 sql_injection 时优先使用此工具
重要: 根据当前节点的漏洞类型(vuln_type)选择匹配的 preset! 例如: vuln_type=sql_injection 时用 preset="sqli", vuln_type=rce 时用 preset="rce"。不要在 SQLi 目标上测试 RCE payload!
...
行动选择优先级（严格遵循）：
1. 【首选】如果 vuln_type=sql_injection 且尚未调用 sqli_detect → sqli_detect(url, param) (最高效的 SQLi 检测，自动执行 14 种错误模式 + 时间盲注 + 布尔盲注)
2. 【首选】如果 vuln_type≠sql_injection 且尚未测试 → batch_inject (用 4-6 个常见 payload 批量初探)
3. 【次选】如果 batch_inject 发现异常 → inject_payload 针对具体异常深挖
4. 【次选】如果发现 WAF/过滤 → probe_filter 探测过滤规则。重要: 若 probe_filter 返回 blocked=[] 且所有 SQL 字符在 allowed 中 → 目标无 WAF, 跳过 mutate_payload, 直接用 UNION SELECT / 堆叠查询 / 报错注入等高级 payload
5. 【次选】如果需要发现隐藏页面/API → crawl_page / render_page / deep_crawl
6. 【次选】如果需要认证测试 → test_no_auth / test_idor
7. 【最后手段】仅当前面所有方法都试过后 → run_poc

不要第一步就用 run_poc！不要连续多次 run_poc！run_poc 是最后手段，不是首选！优先用 inject_payload 做实际注入测试！

【强制规则 — SQLi 节点】：
- vuln_type=sql_injection 时，第一步必须使用 sqli_detect(url, param)，不要先用 batch_inject
- sqli_detect 自动执行错误型(14种模式)、时间盲注(5种数据库)、布尔盲注检测，效率远高于手动注入
- 只有 sqli_detect 返回 “not vulnerable” 后才用 batch_inject preset='sqli' 补充
- 绝对不要在 sql_injection 节点上测试 cmd/exec/id 等 RCE 参数

【重要】漏洞类型专注原则：
- 你当前的任务是测试 **{vuln_type}** 类型的漏洞
- 只使用与 {vuln_type} 相关的 payload，不要测试其他类型的漏洞
- 例如：如果 vuln_type=sql_injection，只测试 SQL 注入 payload，不要测试 RCE/XSS/LFI 等
- 例如：如果 vuln_type=xss，只测试 XSS payload，不要测试 SQL 注入/RCE 等
- 如果发现 batch_inject preset='sqli' 无效，应该尝试其他 SQLi 变体（UNION、报错注入、布尔盲注），而不是切换到其他漏洞类型
...
def build_react_prompt(
    ...
    preset_map = {
        'sql_injection': 'sqli', 'xss': 'xss', 'lfi': 'lfi',
        'rce': 'rce', 'ssti': 'ssti', 'ssrf': 'ssrf',
        'path_traversal': 'lfi', 'open_redirect': 'ssrf',
    }
    suggested_preset = preset_map.get(vuln_type, '')
    if suggested_preset:
        parts.append(f"- 建议使用: batch_inject preset='{suggested_preset}' 或 sqli_detect (如适用)")
    # 嵌入 URL 推断参数信息，避免 Agent 猜测不存在的参数
    current_param = state_dict.get('current_param', '')
    if current_param:
        parts.append(f"- ⚠️ 已知有效参数: '{current_param}' (从 URL 推断)，直接使用此参数测试，不要猜测其他参数名")
    parts.extend([
