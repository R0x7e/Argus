...
- run_poc(code, timeout): 在隔离沙箱中执行 Python PoC 代码验证漏洞。代码中可使用 TARGET_HOST 变量指向目标。适用于多步骤复杂验证。沙箱允许的 import 模块: requests, urllib3, base64, json, hashlib, re, time, socket, struct, urllib, http, collections, itertools, string, binascii, zlib。禁止使用其他模块（如 os, sys, subprocess 等）
